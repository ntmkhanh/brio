...
def base_setting(args):
    ...
    args.model_type = getattr(args, "model_type", "vinai/bartpho-word-base") # model type "vinai/bartpho-syllable"
    ...
    args.pretrained = getattr(args, "pretrained", "./finetuned_model_final/eval_bartpho") # pretrained model path
    ...
    args.max_len = getattr(args, "max_len", 1024) # max length of summary
    ...
    args.total_len = getattr(args, "total_len", 120) # total length of source article
    ...
    args.gen_max_len = getattr(args, "gen_max_len", 1024) # max length of generated summaries
...
def evaluation(args):
    ...
    batch_size = 4
    dataloader = DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=2, collate_fn=collate_fn)
    # build models
...
def run(rank, args):
    ...
    if is_mp:
        ...
        dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, num_workers=2, collate_fn=collate_fn, sampler=train_sampler)
        val_sampler = torch.utils.data.distributed.DistributedSampler(
    	 val_set, num_replicas=world_size, rank=rank)
        val_dataloader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=2, collate_fn=collate_fn_val, sampler=val_sampler)
        val_gen_dataloader = DataLoader(val_set, batch_size=8, shuffle=False, num_workers=2, collate_fn=collate_fn_val, sampler=val_sampler)
    else:
        dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=2, collate_fn=collate_fn)
        val_dataloader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=2, collate_fn=collate_fn_val)
        val_gen_dataloader = DataLoader(val_set, batch_size=4, shuffle=False, num_workers=2, collate_fn=collate_fn_val)
